...
SC_MODULE( Router ) {
            ...
            if(rst.read() == false) {
                ...
                for(int out_port_dir = 0; out_port_dir < 5; out_port_dir++) {
                    ...
                    {  
                        ...
                        if ((out_buf[out_port_dir][32] == 1) && in_ack[out_port_dir].read() == true)
                        {
                    ...
                    else // find new packet
                    {
                        for (int in_buf_dir = 0; in_buf_dir < 5; in_buf_dir++)
                        {
                            ...
                            if (in_req[in_buf_dir].read() == true)
                            {
                                ...
                                if (in_flit[in_buf_dir].read()[33] == 1)
                                {
                                    // Extract information
                                    int dst_id = in_flit[in_buf_dir].read().range(27, 24).to_uint(); 
    ...
    Router(sc_module_name name, int id, sc_trace_file *tf = NULL) : sc_module(name), id(id), tf(tf){
        ...
        // ss0 << "router_" << id << ".clk";
        // sc_trace(tf, clk, ss0.str());
        // ss1 << "router_" << id << ".rst";
        // sc_trace(tf, rst, ss1.str());

        // sc_trace(tf, clk, "router_" + to_string(id) + ".clk");
        // sc_trace(tf, rst, "router_" + to_string(id) + ".rst");
