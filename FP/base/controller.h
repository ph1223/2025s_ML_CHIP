...
SC_MODULE( Controller ) {
    ...
    sc_out < sc_lv<3> > M_ARSIZE; // 000: BYTE, 001: HWORD, 010: WORD, 011: DWORD, 100: QWORD
    ...
    sc_in  < sc_lv<32> > M_RDATA; // AXI Read Data
            ...
            if (rst.read()){
                ...
                switch(state){
                    case 0: {
                        ...
                        if(layer_id_cnt == 0){
                            expected_data_cnt = 150528; // 3 * 224 * 224

                            while(received_data_cnt != expected_data_cnt){
                                if(!is_reading_data){
                                    M_ARADDR.write(data_address);
                                    M_ARLEN = (expected_data_cnt - received_data_cnt) / 128 ? 127 : (expected_data_cnt - received_data_cnt) - 1;
                        ...
                        switch (layer_id_cnt){
                            case (1):
                                expected_weight_cnt = 23232; // 64 * 3 * 11 * 11
                                expected_bias_cnt = 64; // 64
                                break;
                            case (2):
                                expected_weight_cnt = 307200; // 192 * 64 * 5 * 5
                                expected_bias_cnt = 192; // 192
                                break;
                            case (3):
                                expected_weight_cnt = 663552; // 384 * 192 * 3 * 3
                                expected_bias_cnt = 384; // 384
                                break;
                            case (4):
                                expected_weight_cnt = 884736; // 256 * 384 * 3 * 3
                                expected_bias_cnt = 256; // 256
                                break;
                            case (5):
                                expected_weight_cnt = 589824; // 256 * 256 * 3 * 3
                                expected_bias_cnt = 256; // 256
                                break;
                            case (6):
                                expected_weight_cnt = 37748736; // 4096 * 9216
                                expected_bias_cnt = 4096; // 4096
                                break;
                            case (7):
                                expected_weight_cnt = 16777216; // 4096 * 4096
                                expected_bias_cnt = 4096; // 4096
                                break;
                            case (8):
                                expected_weight_cnt = 4096000; // 1000 * 4096
                                expected_bias_cnt = 1000; // 1000
                                break;
                            default:
                        ...
                        while(received_weight_cnt != expected_weight_cnt){
                            if(!is_reading_data){
                                M_ARADDR.write(data_address);
                                M_ARLEN = (expected_weight_cnt - received_weight_cnt) / 128 ? 127 : (expected_weight_cnt - received_weight_cnt) - 1;
                                M_ARVALID.write(true);
                        ...
                        while(received_bias_cnt != expected_bias_cnt){
                            if(!is_reading_data){
                                M_ARADDR.write(data_address);
                                M_ARLEN = (expected_bias_cnt - received_bias_cnt) / 128 ? 127 : (expected_bias_cnt - received_bias_cnt) - 1;
                                M_ARVALID.write(true);
