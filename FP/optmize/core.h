...
SC_MODULE( Core ) {
    ...
    sc_in  < bool >  clk;
    // receive
    sc_in  < sc_lv<130> > flit_rx;	// The input channel
    sc_in  < bool > req_rx;	        // The request associated with the input channel
    sc_out < bool > ack_rx;	        // The outgoing ack signal associated with the input channel
    // transmit
    sc_out < sc_lv<130> > flit_tx;	// The output channel
        ...
        while(true){
            if(rst_n.read() == true){ 
                ...
                if(send_count == 0){
                    ...
                    header.range(123,120) = dest_id;
                    header.range(119,118) = data_type;
                    header.range(117,0) = 0;
                    flit_tx.write(header);
                    send_count++;
                }
                else if(ack_tx.read() == true && req_tx.read() == true){
                    if(send_count == num_packets - 4){
                        float data_val;
                        ...
                        data_flit.range(127,96) = float_to_sc_lv32(data_val);
                        data_val = data_q.front(); 
                        data_q.pop();
                        data_flit.range(95,64) = float_to_sc_lv32(data_val);
                        data_val = data_q.front();
                        data_q.pop();
                        data_flit.range(63,32) = float_to_sc_lv32(data_val);
                        data_val = data_q.front();
                        data_q.pop();
